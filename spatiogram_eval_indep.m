...
params.FinalResultsOutputDir = 'rec_front_small_uniform/spat';
...
for iTestHRTF = 1:20
    for iSplit = 1:length(trainTestRecordings)
        fprintf("Test HRTF %d / %d, Split %d / %d\n", ...
            iTestHRTF, 20, ...
            iSplit, length(trainTestRecordings))
    
        [recordingsTrain, ~, realLocsTrain] = loadRecordings3( ...
            params, trainTestRecordings(iSplit).Train, iTestHRTF);
        assert(size(recordingsTrain, 1) == 608)
        ...
        %% Calculate test POSC
        [recordingsTest, realWidthsTest, realLocsTest] = loadRecordings3( ...
            params, trainTestRecordings(iSplit).Test, iTestHRTF);
        ...
        finalResults(iTestHRTF, iSplit).RealTest = realWidthsTest;
        finalResults(iTestHRTF, iSplit).PredTest = predWidths;
        finalResults(iTestHRTF, iSplit).TrainRecordings = trainTestRecordings(iSplit).Train;
        finalResults(iTestHRTF, iSplit).TestRecordings = trainTestRecordings(iSplit).Test;
        finalResults(iTestHRTF, iSplit).TestMean = mean(abs(realWidthsTest - predWidths));
        finalResults(iTestHRTF, iSplit).TestStd = std(abs(realWidthsTest - predWidths));
        finalResults(iTestHRTF, iSplit).BestParams = bestResult;
        finalResults(iTestHRTF, iSplit).RealLocsTest = realLocsTest;
        finalResults(iTestHRTF, iSplit).RealLocsTrain = realLocsTrain;
        
        fprintf("Test MAE = %.2f\n", finalResults(iTestHRTF, iSplit).TestMean)
        fprintf("Test STD = %.2f\n", finalResults(iTestHRTF, iSplit).TestStd)
    end
end
...
function [recordings, realWidths, realLocs] = loadRecordings3(params, ...
    requestRecordingNames, requestHRTF)
rng(10);
...
HRTFIDs = filenameParts(:, :, 2);
requestHRTFname = "hrtf" + requestHRTF;
requestMask = cellfun(@(c) any(ismember(requestRecordingNames, c)), recordingNames);
requestMask = requestMask & cellfun(@(c) any(ismember(requestHRTFname, c)), HRTFIDs);
